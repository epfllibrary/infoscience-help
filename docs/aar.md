...
*A comprehensive **documentation** for the AAR is also available (authentication required) at: [inside.epfl.ch/data/aar](https://inside.epfl.ch/data/aar)*
...
[Infoscience](https://infoscience.epfl.ch/) is the institutional repository of EPFL: its purpose is to collect, preserve, and enhance the entire academic and scientific output associated with EPFL.
...
![Access button AAR in Sesame](assets/images/aar/acces-aar.png)

- **Login:** The academic report is accessible on the [Sesame portal](https://sesame.epfl.ch/), Projects section, AAR Validation tile;
- **Access** is automatic for unit heads based on ACCRED rights. If the button does not appear, contact [1234@epfl.ch](mailto:1234@epfl.ch) ;
- **Delegation of Rights:** unit heads can delegate access to other collaborators via [accred.epfl.ch](http://accred.epfl.ch/), using the authorization: "AAR – see academic achievement reports" ;
...
The complete list of document types accepted in Infoscience is [available here](https://go.epfl.ch/help-infoscience-types).
...
![Section Publications Academic Report AAR](assets/images/aar/publication-visualisationAAR.png)
...
![AAR validation menu — Confirm button](assets/images/aar/AAR_menuVerification.png)
...
<span id="faq-q1"></span>
??? question "Q1 — Which AAR sections are fed by Infoscience and which are not?"
    In practice, *Publications* and *Patents* come from Infoscience (bibliographic and attribution data).
    The rest depends on other EPFL sources (HR, IS-ACADEMIA, GrantsDB, etc.) and is not managed by Infoscience.

<span id="faq-q2"></span>
??? question "Q2 — Where do the publications and patents listed in the academic report come from?"
    [Infoscience](https://infoscience.epfl.ch/home) is the single source of truth for the "Publications" and "Patents" data used in the Annual Academic Report (AAR). Therefore, **any correction or update must be performed directly in Infoscience before being reflected in the AAR**.
...
<span id="faq-q4"></span>
??? question "Q4 — Are EPFL doctoral theses included in the AAR, and according to what rules?"
    Yes. Doctoral theses defended at EPFL referenced in Infoscience are included in the Annual Academic Report (AAR).

<span id="faq-q5"></span>
??? question "Q5 — How is data transmitted from Infoscience to the AAR?"
    [Infoscience](https://infoscience.epfl.ch/home) is fed by a combination of submissions made by researchers, professors, and EPFL units, and automated collection and enrichment processes, carried out daily from reference bibliographic sources (Web of Science, Scopus, OpenAlex, Crossref and Zenodo) and the European Patent Office (Espacenet) for patents, based on the presence of the EPFL affiliation.
...
<span id="faq-q6"></span>
??? question "Q6 — What exactly does the "Written at" field mean?"
...
<span id="faq-q7"></span>
??? question "Q7 — Why does "Written at" not indicate EPFL even though the author is affiliated?"
    ...
    If the EPFL affiliation is indeed mentioned in the publication, a [correction request](https://go.epfl.ch/help-infoscience-correction) can be made so that the "Written at" field is adjusted.

<span id="faq-q8"></span>
??? question "Q8 — How does the process of attributing publications or patents to EPFL units work?"
    ...
    To modify an attribution to your unit in Infoscience, follow the procedure [Add or Reject the Attribution of a Unit to a Publication](https://go.epfl.ch/help-infoscience-unit-add-reject):
    ...
    To submit a publication not yet present on Infoscience: [Submit a Publication](https://go.epfl.ch/help-infoscience-submit)
    
    !!! info ""
        Modifications made and validated in Infoscience are generally visible in the AAR on Day+1.

<span id="faq-q9"></span>
??? question "Q9 — Why is a publication linked to a parent unit (faculty, institute) instead of my unit (laboratory, group)?"
    ...
    If the publication genuinely concerns your laboratory or group, the unit heads or delegated individuals can request or perform an attribution adjustment in Infoscience, to link the publication to the appropriate level 4 unit ([Request a Correction](https://go.epfl.ch/help-infoscience-correction) or [Add or Reject an Attribution](https://go.epfl.ch/help-infoscience-unit-add-reject)).
...
??? question "Q10 — What to do if a publication or patent is not yet linked to my unit?"
    ...
    **Procedure:**
    ...
    For more details, see: [Add or Reject an Attribution](https://go.epfl.ch/help-infoscience-unit-add-reject)
    
    Otherwise (insufficient rights or a situation requiring validation), you can make a [correction request](https://go.epfl.ch/help-infoscience-correction) or contact the Infoscience team via [infoscience@epfl.ch](mailto:infoscience@epfl.ch) or via an AAR incident.

<span id="faq-q11"></span>
??? question "Q11 — Why is a publication attributed with a Non-EPFL unit?"
    ...
    To correct this type of case, refer to the question: [What to do to correct a publication associated with a Non-EPFL unit?](#faq-q20)
...
??? question "Q12 — How to check the list of publications attributed to my unit that will be transmitted in the AAR?"
    ...
    To access it, use the menu [Explore → EPFL Units](https://infoscience.epfl.ch/explore/orgunits), then search for your unit using its acronym.
    ...
    [Example query](https://infoscience.epfl.ch/entities/orgunit/d74be593-dcf3-4e4d-a5ed-9d0304f2831c?spc.page=1&query=dateIssued.year:2025%20AND%20epfl.writtenAt:(EPFL)%20)
...
??? question "Q13 — After a correction request, when will the modification be visible in the AAR?"
...
??? question "Q14 — How to remove a publication incorrectly linked to my unit?"
    It is not possible to delete a publication from Infoscience. In accordance with [the Infoscience deposit license](https://go.epfl.ch/help-infoscience-conditions), accepted records cannot be deleted.
    ...
    - **You have unit management rights (head or delegate):** you can remove the attribution via the option "[*Edit Unit/Lab affiliations*](https://go.epfl.ch/help-infoscience-unit-add-reject)" in the Infoscience record;
    - **You do not have the necessary rights or the situation is complex:** create an incident via the section concerned in the AAR (*Create a new incident*) or make a [correction request](https://go.epfl.ch/help-infoscience-correction) in Infoscience, so that an adjustment is performed at the source.
...
??? question "Q15 — How to report a problem or anomaly in the AAR?"
...
??? question "Q16 — What to do if a publication or patent in the AAR should not be linked to my unit?"
    ...
    Otherwise, you can create an AAR incident or contact [infoscience@epfl.ch](mailto:infoscience@epfl.ch), briefly explaining why the attribution is incorrect.
    
    See the procedures: [Request a Correction](https://go.epfl.ch/help-infoscience-correction) or [Add or Reject an Attribution](https://go.epfl.ch/help-infoscience-unit-add-reject)

<span id="faq-q17"></span>
??? question "Q17 — How to correct the metadata of a publication or patent in the AAR (e.g., title, author, abstract, date)?"
    ...
    See: [Request a Correction](https://go.epfl.ch/help-infoscience-correction) or [Create a New Version](https://go.epfl.ch/help-infoscience-new-version)
    
    Modifications made and validated in Infoscience are generally visible in the AAR on Day+1.

<span id="faq-q18"></span>
??? question "Q18 — Is it possible to correct an attribution directly in Infoscience?"
    Yes, if you have the **necessary rights** (unit head or delegated person), you can correct an attribution **directly in Infoscience**, without going through support ([*Modification Rights and Roles in Infoscience*](https://go.epfl.ch/aar-infoscience-modification-rights)).
    ...
    More details in the help section [*Add or Reject the Assignment of a Unit to a Publication*](https://go.epfl.ch/help-infoscience-unit-add-reject).
    
    Changes are taken into account in Infoscience and will be reflected in the AAR after update (generally on Day+1).
    
    In other cases (insufficient rights, complex situation, or requiring validation), you should submit a correction request. If this is not possible, you can send your request by ticket to [infoscience@epfl.ch](mailto:infoscience@epfl.ch) or create a new AAR incident so that the Infoscience team analyzes and applies the modification.

<span id="faq-q19"></span>
??? question "Q19 — What to do if a publication appears as a duplicate?"
    Report the duplicate via a [correction request](https://go.epfl.ch/help-infoscience-correction) or contact the Infoscience team using an AAR incident or via [infoscience@epfl.ch](mailto:infoscience@epfl.ch), indicating both concerned Infoscience records. The Infoscience team is responsible for merging them.

<span id="faq-q20"></span>
??? question "Q20 — What to do to correct a publication associated with a Non-EPFL unit?"
    ...
    - if the publication does concern EPFL, but is incorrectly linked to a Non-EPFL unit, you can:
        ...
        - make a [correction request](https://go.epfl.ch/help-infoscience-correction) in Infoscience by justifying the EPFL affiliation (if your rights allow it);
        - or contact the Infoscience team via an AAR incident or by [infoscience@epfl.ch](mailto:infoscience@epfl.ch).
...
??? question "Q22 — Why is my publication neither in Infoscience nor in the AAR?"
    Several reasons can explain this:
    
    - the publication has not yet been submitted or imported into Infoscience;
    - the metadata from external sources does not clearly identify an EPFL author or an EPFL affiliation (non-normalized affiliation, ambiguity on author names, absence of [ORCID](https://www.epfl.ch/campus/library/services-researchers/orcid/), etc.).
    
    To limit these situations, it is strongly recommended:
    
    - to create and use an [ORCID](https://www.epfl.ch/campus/library/services-researchers/orcid/) and systematically transmit it to the publisher when submitting an article;
    - to indicate a standardized EPFL affiliation, associating the [EPFL ROR identifier](https://ror.org/02s376052) with it, in addition to the textual mention of the affiliation. *Example: École Polytechnique Fédérale de Lausanne, Lausanne, Switzerland. [https://ror.org/02s376052](https://ror.org/02s376052)*
...
??? question "Q23 — Why does my publication not appear in the AAR even though it is present in Infoscience?"
    ...
    If an anomaly is noted, you can [request a correction in Infoscience](https://go.epfl.ch/help-infoscience-correction) or report the problem via an AAR incident, so that a correction is performed at the source.

<span id="faq-q24"></span>
??? question "Q24 — Why are some expected patents not appearing in the AAR?"
    ...
    If your patent is indeed published and does not yet appear in Infoscience, please contact [infoscience@epfl.ch](mailto:infoscience@epfl.ch) or report a problem via an AAR incident.

<span id="faq-q25"></span>
??? question "Q25 — How to add a missing publication?"
    After checking that the publication does not already exist in Infoscience (search by title, DOI, or author), you can add it directly via the Infoscience submission form, by following the dedicated procedure [*Submit a Publication*](https://go.epfl.ch/help-infoscience-submit).
    ...
    If the volume of missing publications is significant (for example, as part of a catch-up or an earlier period), it is recommended to contact Infoscience support ([infoscience@epfl.ch](mailto:infoscience@epfl.ch)). The team will analyze the situation and evaluate the possibilities for retrospective import or grouped processing.
...
??? question "Q26 — Who has the right to request corrections in Infoscience?"
    Submitters of a record and authors of a publication can:
    
    - [request corrections](https://go.epfl.ch/help-infoscience-correction).
    
    Unit heads or delegated individuals (e.g., administrative assistants) can:
    
    - [request corrections](https://go.epfl.ch/help-infoscience-correction)
    - [modify the attributions](https://go.epfl.ch/help-infoscience-unit-add-reject) of their unit.
    
    The detail of the rights is available here: [*Modification Rights and Roles in Infoscience*](https://go.epfl.ch/aar-infoscience-modification-rights).
    
    Corrections are examined and validated by the Infoscience team (and by the TTO for patents).

<span id="faq-q27"></span>
??? question "Q27 — How to obtain authorization to associate a unit with a publication (or request a modification)?"
    ...
    In case of doubt or a complex situation, contact [infoscience@epfl.ch](mailto:infoscience@epfl.ch).
